...
一个标志，指示是否应该运行 [__destruct()](#__destruct) 中的清理函数。
...
## __construct($connection_options)
...
$conn->queryRange('SELECT * FROM {node} WHERE nid = :nid', 1, 5, array(':nid' => 1));

// 生成的SQL语句如下:
// SELECT * FROM `node` WHERE nid = :nid LIMIT 1, 5
```

详细参见 [DatabaseConnection::queryRange()](../DatabaseConnection.html#queryRange) 。


## queryTemporary($query, $args, $options)

运行一个查询，将其结果存储在临时表中，并返回临时表的名称。
...
```php
$conn = Database::getConnection();
$table = $conn->queryTemporary('SELECT * FROM {node} WHERE nid > :nid', array(':nid' => 1));
$query = $conn->query('SELECT nid, title FROM {' . $table . '}')->fetchAll();
```

详细参见 [DatabaseConnection::queryTemporary()](../DatabaseConnection.html#queryTemporary) 。
...
详细参见 [DatabaseConnection::nextId()](../DatabaseConnection.html#nextId) 。
...
详细参见 [DatabaeConnection::setPrefix()](../DatabaseConnection.html#setPrefix) 。
